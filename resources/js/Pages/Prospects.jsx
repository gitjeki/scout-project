            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-orange-200">
                <div className="bg-orange-600 p-4 text-white flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <div><h3 className="font-bold text-lg">Call #{prospect.prospect_id}</h3></div>
                    </div>
...
const SalesRow = ({ item, onCallClick }) => {
    ...
    return (
        <tr className="hover:bg-gray-50 transition-colors border-b border-gray-100 group">
            ...
            <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">{item.last_contact_at}</td>
            <td className="px-4 py-3 font-mono text-xs">
                {item.total_duration_sec > 0 ? (
                    <span className="text-purple-700 font-bold bg-purple-50 px-2 py-1 rounded">{formatDuration(item.total_duration_sec)}</span>
...
export default function Prospects({ prospects, statusOptions, channelOptions, filters = {} }) {
