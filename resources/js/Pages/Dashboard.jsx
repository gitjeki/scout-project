...
export default function Dashboard({ stats, prospects, statusOptions = [], scoringUsers = [], filters = {}, personalStats, personalPipelineStats, globalPipelineStats, formTemplate }) {
    ...
    const handleFilterChange = (key, value) => {
        ...
        // --- LOGIC PERUBAHAN: PAKSA SORT KE SCORE DESC JIKA ADA FILTER BERUBAH ---
        // Mirip seperti behavior dashboard awal
        const newSortField = 'score';
        const newSortDirection = 'desc';

        // Update State Sort Lokal
        setSortField('score');
        ...
        router.get(route('dashboard'), { 
            ...
            search_id: searchId, 
            sort_field: newSortField, // KIRIM SCORE
            sort_direction: newSortDirection // KIRIM DESC
    ...
    const handleResetFilters = () => {
        ...
        setFilterScoredAt('');
        setSearchId('');
        // Reset ke halaman dashboard tanpa parameter query (Default di Controller biasanya Score DESC)
        router.get(route('dashboard'));
    };
    ...
    return (
        <SidebarLayout header={isSales ? "Sales Dashboard" : "Administrator Dashboard"}>
            ...
            {/* --- ADMIN & TABLE SECTION (COMPACT LAYOUT) --- */}
            {isAdmin && (
                <>
                {/* 1. FILTER SECTION (COMPACT & REORGANIZED) */}
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
                    
                    {/* HEADER: Judul Kiri, Tombol Bulk Kanan */}
                    <div className="flex flex-wrap justify-between items-end mb-3 gap-2">
                        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                            <FaFilter/> Filter & Search Data
                        </h3>

                        {/* MOVED BULK ACTIONS HERE */}
                        <div className="flex gap-2">
                            <button 
                                onClick={toggleSelectAllMatching}
                                className={`flex items-center justify-center gap-2 px-3 py-1.5 rounded-md text-[10px] font-bold transition border shadow-sm
                                    ${selectAllMatching 
                            ...
                            >
                                ...
                                {selectAllMatching ? 'Cancel Select' : `Select All (${prospects.total})`}
                            </button>

                            <button 
                                onClick={requestDeleteSelection} 
                                disabled={selectedIds.length === 0 && !selectAllMatching}
                                className={`flex items-center justify-center gap-2 px-3 py-1.5 rounded-md text-[10px] font-bold transition shadow-sm
                    ...
                    {/* GRID SYSTEM: 3 KOLOM (Tanpa Margin Bawah Berlebih) */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        {/* --- ROW 1 --- */}
                        
                        {/* 1. Search ID */}
                        <form onSubmit={handleSearchId} className="relative w-full">
                        ...
                        <div className="relative w-full">
                             <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-xs"><FaFilter size={10}/></div>
                             <select value={filterStatus} onChange={(e) => handleFilterChange('status', e.target.value)} className="pl-8 pr-3 py-2 border-gray-300 rounded-lg text-xs w-full focus:ring-blue-500 focus:border-blue-500 shadow-sm appearance-none bg-white">
                        ...
                        {/* 3. Priority Filter */}
                        <div className="relative w-full">
                            <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-xs"><FaFire size={10}/></div>
                            <select value={filterPriority} onChange={(e) => handleFilterChange('priority', e.target.value)} className="pl-8 pr-3 py-2 border-gray-300 rounded-lg text-xs w-full focus:ring-blue-500 focus:border-blue-500 shadow-sm appearance-none bg-white">
                        ...
                        {/* 4. Scored By */}
                         <div className="relative w-full">
                            <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-xs"><FaUser size={10}/></div>
                            <select value={filterScoredBy} onChange={(e) => handleFilterChange('scored_by', e.target.value)} className="pl-8 pr-3 py-2 border-gray-300 rounded-lg text-xs w-full focus:ring-blue-500 focus:border-blue-500 shadow-sm appearance-none bg-white">
                                <option value="">Scored By (All)</option>
                        ...
                        {/* 5. Scored At */}
                        <div className="relative w-full">
                             <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-xs"><FaClock size={10}/></div>
                             <select value={filterScoredAt} onChange={(e) => handleFilterChange('scored_at', e.target.value)} className="pl-8 pr-3 py-2 border-gray-300 rounded-lg text-xs w-full focus:ring-blue-500 focus:border-blue-500 shadow-sm appearance-none bg-white">
                                <option value="">Scored Time (All)</option>
                        ...
                        <button 
                            ...
                            className="w-full py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 border border-gray-300 rounded-lg transition flex items-center justify-center gap-2 font-bold text-xs shadow-sm"
                        >
