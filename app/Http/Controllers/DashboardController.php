...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $stats = [
            ...
            'today_input' => Prospect::whereDate('created_at', Carbon::now('Asia/Jakarta'))->count(),
            'total_predicted' => PredictionScore::count(),
            'today_predicted' => PredictionScore::whereDate('scored_at', Carbon::now('Asia/Jakarta'))->count(),
        ];
        ...
        if ($isSales) {
            ...
            $callsToday = (clone $myCallsQuery)->whereDate('contact_at', Carbon::now('Asia/Jakarta'))->count();
            $durationTotal = (clone $myCallsQuery)->whereDate('contact_at', Carbon::now('Asia/Jakarta'))->sum('call_duration_sec');
        ...
        $query = Prospect::with(['status', 'latestScore.user'])
            ->withCount('contactActivities as call_count')
            ->readyForPrediction(); 
        ...
        $statusOptions = ProspectStatus::select('status_code')->distinct()->orderBy('status_code')->pluck('status_code');
        ...
        $prospects = $query->paginate(10)
            ...
            ->through(function ($item) {
                ...
                return [
                    ...
                    // Tetap parse ke Asia/Jakarta untuk tampilan frontend yang aman
                    'scored_at'      => $item->latestScore 
                                            ? Carbon::parse($item->latestScore->scored_at)
                    ...
                    'last_contact_at'    => $item->latestActivity 
                                                ? Carbon::parse($item->latestActivity->contact_at)
                                                    ->setTimezone('Asia/Jakarta')
                                                    ->diffForHumans() 
                                                : '-',
    ...
    /**
     * Run Predictions - PERBAIKAN TIMEZONE DI SINI
     */
    public function runPredictions(Request $request)
    {
                ...
                if ($response->successful()) {
                    ...
                    foreach ($results as $res) {
                        try {
                            ...
                            // PERUBAHAN UTAMA: Gunakan Asia/Jakarta saat simpan scored_at
                            PredictionScore::updateOrCreate(
                                ...
                                [
                                    ...
                                    'scored_at'         => Carbon::now('Asia/Jakarta'), 
                                ]
                            );
                            $totalProcessed++;
                        } catch (\Exception $e) {
        ...
        if ($totalProcessed > 0) {
            return redirect()->route('dashboard')
                ->with('success', "Proses selesai. {$totalProcessed} data berhasil diprediksi (WIB).");
        } else {
            return redirect()->route('dashboard')
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $csv = array_map(fn($line) => str_getcsv($line, $delimiter), file($path));

            if (empty($csv)) return back()->with('error', 'File CSV kosong.');

            $header = array_map(fn($col) => strtolower(trim($col)), array_shift($csv));
            $columnMapping = [
                'age' => 'age', 'job' => 'job', 'education' => 'education', 'month' => 'month',
                'duration' => 'duration', 'campaign' => 'campaign', 'poutcome' => 'poutcome',
                'cons.price.idx' => 'cons_price_idx', 'cons_price_idx' => 'cons_price_idx',
                'cons.conf.idx' => 'cons_conf_idx', 'cons_conf_idx' => 'cons_conf_idx',
                'euribor3m' => 'euribor3m', 'nr.employed' => 'nr_employed', 'nr_employed' => 'nr_employed',
            ];

            $headerIndexes = [];
            foreach ($header as $index => $columnName) {
                if (isset($columnMapping[$columnName])) {
                    $headerIndexes[$columnMapping[$columnName]] = $index;
                }
            }

            $defaultStatus = ProspectStatus::firstOrCreate(['status_code' => 'NEW'], ['status_type' => 'open']);
            $imported = 0;
            ...
            foreach ($csv as $row) {
                ...
                foreach ($headerIndexes as $dbColumn => $csvIndex) {
                    ...
                    $data[$dbColumn] = ($val === '' || strtolower($val) === 'unknown') ? null : $val;
                }
            ...
            DB::commit();
            return back()->with('success', "Import selesai: {$imported} data berhasil.");
        } catch (\Exception $e) {
            DB::rollBack();
